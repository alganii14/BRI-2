...
@section('title', 'Penurunan Brilink')
@section('page-title', 'Data Penurunan Brilink')
...
<div class="header-actions">
    <div style="display: flex; gap: 10px;">
        ...
        <a href="{{ route('penurunan-brilink.import.form') }}" class="btn btn-success">
            📁 Import CSV
        </a>
    </div>
...
<div class="table-container">
    <table>
        ...
        <tbody>
            @forelse($penurunanBrilinks as $index => $item)
            <tr>
                <td>{{ $penurunanBrilinks->firstItem() + $index }}</td>
                ...
                <td>
                    <div class="actions">
                        <a href="{{ route('penurunan-brilink.show', $item->id) }}" class="btn btn-sm btn-info">👁️ View</a>
                        <a href="{{ route('penurunan-brilink.edit', $item->id) }}" class="btn btn-sm btn-warning">✏️ Edit</a>
                        <form action="{{ route('penurunan-brilink.destroy', $item->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
            ...
            <tr>
                <td colspan="13" style="text-align: center; padding: 40px;">
                    <p style="color: #999; font-size: 16px;">Tidak ada data penurunan brilink.</p>
                    <a href="{{ route('penurunan-brilink.import.form') }}" class="btn btn-success" style="margin-top: 10px;">Import CSV</a>
                </td>
            </tr>
            @endforelse
        </tbody>
...
<div class="pagination-wrapper">
    <p class="pagination-info">Showing {{ $penurunanBrilinks->firstItem() }} to {{ $penurunanBrilinks->lastItem() }} of {{ $penurunanBrilinks->total() }} results</p>
    
    <div style="display: flex; justify-content: center; gap: 10px; margin-top: 15px; flex-wrap: wrap;">
        @if ($penurunanBrilinks->onFirstPage())
        ...
        @else
            <a href="{{ $penurunanBrilinks->previousPageUrl() }}" style="padding: 10px 20px; background: white; color: #333; border: 1px solid #ddd; border-radius: 4px; cursor: pointer; text-decoration: none;">← Previous</a>
        @endif

        {{-- Show pages 1 to 5 only --}}
        @php
            $currentPage = $penurunanBrilinks->currentPage();
            $lastPage = $penurunanBrilinks->lastPage();
            $startPage = 1;
            $endPage = min(5, $lastPage);
        @endphp

        @foreach (range($startPage, $endPage) as $page)
            @php $url = $penurunanBrilinks->url($page); @endphp
            @if ($page == $currentPage)
        ...
        @if ($penurunanBrilinks->hasMorePages())
            <a href="{{ $penurunanBrilinks->nextPageUrl() }}" style="padding: 10px 20px; background: white; color: #333; border: 1px solid #ddd; border-radius: 4px; cursor: pointer; text-decoration: none;">Next →</a>
        @else
