...
@section('page-title')
Data Penurunan Brilink - KC {{ auth()->user()->nama_kanca }}
@endsection
...
<div class="card">
    <div style="padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px 12px 0 0;">
        <h3 style="color: white; margin: 0;">📊 Data Penurunan Brilink</h3>
    ...
    <div style="padding: 20px;">
        <form method="GET" action="{{ route('manager.pipeline.brilink') }}" class="search-box">
            ...
            @if($search || request('month') || request('year'))
                <a href="{{ route('manager.pipeline.brilink') }}" class="btn-search" style="background: #6c757d;">Reset</a>
            @endif
        </form>
    </div>
            ...
            <tbody>
                @forelse($data as $index => $item)
                    <tr>
                        ...
                        <td><span style="display: inline-block; background: #e3f2fd; color: #1565c0; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;">{{ $item->jenis_simpanan ?? 'N/A' }}</span></td>
                    ...
                    <tr>
                        <td colspan="11" style="text-align: center; padding: 40px; color: #666;">
                            ...
                            <p style="margin: 0;">
                                ...
                                @else
                                    Belum ada data penurunan brilink untuk KC Anda
                                @endif
                            </p>
                        </td>
                    </tr>
    ...
    @if($data->hasPages())
        <div style="margin-top: 20px; display: flex; justify-content: center; gap: 8px; padding: 20px;">
            ...
            @else
                <a href="{{ $data->previousPageUrl() }}" class="btn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none;">← Sebelumnya</a>
            @endif
            ...
            @foreach (range(1, min(5, $lastPage)) as $page)
                @if ($page == $data->currentPage())
                    <span class="btn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 8px 16px; border-radius: 6px;">{{ $page }}</span>
                @else
                    <a href="{{ $data->url($page) }}" class="btn" style="background: #f5f5f5; color: #667eea; padding: 8px 16px; border-radius: 6px; text-decoration: none;">{{ $page }}</a>
                @endif
            @endforeach

            {{-- Next Button --}}
            @if($data->hasMorePages())
                <a href="{{ $data->nextPageUrl() }}" class="btn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none;">Selanjutnya →</a>
            @else
