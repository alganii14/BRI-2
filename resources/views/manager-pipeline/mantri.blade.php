...
@section('page-title')
Data Penurunan Mantri - KC {{ auth()->user()->nama_kanca }}
@endsection
...
<style>
    ...
    th { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; font-weight: 600; font-size: 14px; }
    tr:hover { background-color: #f5f5f5; }
    .search-box { margin-bottom: 20px; display: flex; gap: 10px; }
    .search-box input { flex: 1; padding: 10px 16px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; }
    .btn-search { padding: 10px 24px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; }
</style>

<div class="card">
    <div style="padding: 20px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border-radius: 12px 12px 0 0;">
        <h3 style="color: white; margin: 0;">🏪 Data Penurunan Mantri</h3>
            ...
            <tbody>
                @forelse($data as $index => $item)
                    <tr>
                        ...
                        <td><span style="display: inline-block; background: #fce4ec; color: #c2185b; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;">{{ $item->jenis_simpanan ?? 'N/A' }}</span></td>
                ...
                @empty
                    <tr>
                        <td colspan="11" style="text-align: center; padding: 40px; color: #666;">
                            <p>{{ $search ? "Tidak ada data yang cocok dengan pencarian \"$search\"" : 'Belum ada data penurunan mantri untuk KC Anda' }}</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if($data->hasPages())
        <div style="margin-top: 20px; display: flex; justify-content: center; gap: 8px; padding: 20px;">
            ...
            @else
                <a href="{{ $data->previousPageUrl() }}" class="btn" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none;">← Sebelumnya</a>
            @endif
            ...
            @foreach (range(1, min(5, $lastPage)) as $page)
                @if ($page == $data->currentPage())
                    <span class="btn" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 8px 16px; border-radius: 6px;">{{ $page }}</span>
                @else
                    <a href="{{ $data->url($page) }}" class="btn" style="background: #f5f5f5; color: #f093fb; padding: 8px 16px; border-radius: 6px; text-decoration: none;">{{ $page }}</a>
                @endif
            @endforeach

            {{-- Next Button --}}
            @if($data->hasMorePages())
                <a href="{{ $data->nextPageUrl() }}" class="btn" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none;">Selanjutnya →</a>
            @else
