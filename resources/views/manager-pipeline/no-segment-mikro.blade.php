...
<div class="card">
    ...
    @if($data->hasPages())
        <div style="margin-top: 20px; display: flex; justify-content: center; gap: 8px; padding: 20px;">
            ...
            @else
                <a href="{{ $data->previousPageUrl() }}" class="btn" style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); color: #333; padding: 8px 16px; border-radius: 6px; text-decoration: none;">← Sebelumnya</a>
            @endif
            @php $lastPage = $data->lastPage(); @endphp
            @foreach (range(1, min(5, $lastPage)) as $page)
                @if ($page == $data->currentPage())
                    <span class="btn" style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); color: #333; padding: 8px 16px; border-radius: 6px;">{{ $page }}</span>
                @else
                    <a href="{{ $data->url($page) }}" class="btn" style="background: #f5f5f5; color: #a8edea; padding: 8px 16px; border-radius: 6px; text-decoration: none;">{{ $page }}</a>
                @endif
            @endforeach
            @if($data->hasMorePages())
                <a href="{{ $data->nextPageUrl() }}" class="btn" style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); color: #333; padding: 8px 16px; border-radius: 6px; text-decoration: none;">Selanjutnya →</a>
            @else
